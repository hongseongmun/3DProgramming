...
void Update()
{
    while (!glfwWindowShouldClose(window))
    {
        ...
        transform.translate = transform.translate * moveMat;
        transform.rotation = transform.rotation * rotation ;
        
        cout << transform.scale.num[0][0];
        ...
        if (scaleState)
        {
            transform.scale = transform.scale * downMove;
            x = x * mx;
        }
        else if (!scaleState)
        {
            transform.scale =transform.scale * upMove;
            x = x * px;
        }
