...
float nowScale = 1.00f;
float baseScale = 1.01f;
float baseScale2 = 0.99f;

glm::mat3 leesangjun = glm::mat3(
    1, 0, 0,
    0, 1, 0,
    0.001, 0, 1
);
      
glm::mat3 songjieun = glm::mat3(
    glm::cos(glm::radians(0.0f)), -glm::sin(glm::radians(1.0f)), 0,
    glm::sin(glm::radians(1.0f)), glm::cos(glm::radians(0.0f)), 0,
    0, 0, 1
);

glm::mat3 upHongseongmun = glm::mat3(
    1.01, 0, 0,
    0, 1.01, 0,
    0, 0, 1
);

glm::mat3 downHongseongmun = glm::mat3(
    0.99, 0, 0,
    0, 0.99, 0,
    0, 0, 1
);
...
void Update()
{
    while (!glfwWindowShouldClose(window))
    {
        ...
        transform.translate = transform.translate * leesangjun;
        transform.rotation = transform.rotation * songjieun;

        if (scaleState == false)
        {
            transform.scale = transform.scale * upHongseongmun;
            nowScale = nowScale * baseScale;
            if (nowScale >= 1.3)
            {
                scaleState = true;
            }
        ...
        else if(scaleState == true)
        {
            
            transform.scale = transform.scale * downHongseongmun;
            nowScale = nowScale * baseScale2;
             if (nowScale <= 0.7)
             {
                scaleState = false;
             }
